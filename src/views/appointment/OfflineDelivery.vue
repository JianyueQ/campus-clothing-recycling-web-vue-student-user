<template>
  <div class="offline-delivery">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain :icon="Plus" @click="handleAdd">我要投递</el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table v-loading="loading" :data="tableData" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="取件地址" align="center" prop="pickupAddress" width="200px"/>
      <el-table-column label="联系电话" align="center" prop="phoneNumber" width="150px"/>
      <el-table-column label="取件状态" align="center" prop="pickupStatus" width="100px">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          <el-button link type="primary" :icon="EditPen" @click="handleUpdate(scope.row)">修改</el-button>
          <el-button link type="danger" :icon="Delete" @click="handleDelete(scope.row)">删除</el-button>
          <el-button link type="danger" :icon="Delete" @click="handleUpdateStatus(scope.row)" :disabled="scope.row.pickupStatus === 3">取消</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" width="50%">
      <el-form :model="dialogForm" label-width="120px" :rules="rules" ref="dialogFormRef">
        <el-form-item label="取件地址" prop="pickupAddress">
          <el-input v-model="dialogForm.pickupAddress" placeholder="请输入详细的取件地址"></el-input>
        </el-form-item>
        <el-form-item label="联系电话" prop="phoneNumber">
          <el-input v-model="dialogForm.phoneNumber" placeholder="请输入联系电话"></el-input>
        </el-form-item>
        <el-form-item label="衣物图片" prop="imagesUrl">
          <el-upload
              :multiple="true"
          ...
          <!--          <div class="el-upload__tip">只能上传jpg/png文件，且不超过5张</div>-->
        </el-form-item>
      </el-form>
      ...
      <el-form label-width="120px">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item>
              <template #label>
                <span class="bold-label">衣物图片:</span>
              </template>
              <div v-if="detailData.imagesUrl && detailData.imagesUrl.length > 0"
                   class="image-preview photo-wall">
                <div v-for="(url, index) in detailData.imagesUrl.slice(0, 5)"
                     :key="index"
                  ...
                  <el-image style="width: 100px; height: 100px"
                            ...
                            :max-scale="7"
                            :min-scale="0.2"
                            :preview-src-list="detailData.imagesUrl.map(img => img + '?t=' + new Date().getTime())"
                            :initial-index="index"
                            fit="cover"
                  />
                </div>
...
import {
  addOrderOfflineService,
  deleteOrderService,
  cancelOrderService,
  getOrderByIdOfflineService,
  getOrderListOfflineService,
  updateOrderOfflineService,
} from "@/api/order.js";
...
const getList = async () => {
  loading.value = true;
  try {
    const res = await getOrderListOfflineService(queryParams);
...
const handleAdd = () => {
  ...
  fileList.value = []; // 确保 fileList 也被重置
...
const handleUpdate = async (row) => {
  ...
  if (row) {
    const response = await getOrderByIdOfflineService(row.id);
    Object.assign(dialogForm, response.data);
    // 初始化 fileList，用于显示已上传的图片，直接使用原始 URL
    fileList.value = dialogForm.imagesUrl.map(url => ({
      url: url, // 直接使用原始 URL
      name: '衣物图片' // 可以根据需要设置文件名
    }));
  } else {
    // 如果是通过多选操作触发且仅选中一行，则取第一个元素作为数据源
    if (multipleSelection.value.length === 1) {
      const response = await getOrderByIdOfflineService(multipleSelection.value[0].id);
      Object.assign(dialogForm, response.data);
      // 初始化 fileList，用于显示已上传的图片，直接使用原始 URL
      fileList.value = dialogForm.imagesUrl.map(url => ({
        url: url, // 直接使用原始 URL
...
const handleDialogSubmit = async () => {
  try {
    ...
    const payload = {
      ...dialogForm,
      imagesUrl: Array.isArray(dialogForm.imagesUrl)
          ? dialogForm.imagesUrl
          : dialogForm.imagesUrl.split(',').map(url => url.trim())
    };

    let result;
    if (dialogType.value === 'add') {
      // result = await addOrderOfflineService(dialogForm);
      result = await addOrderOfflineService(payload);
      ElMessage.success('新增成功');
    } else if (dialogType.value === 'edit') {
      result = await updateOrderOfflineService(dialogForm);
...
const imageUploadUrl = ref('/api/common/upload');

const handleUploadSuccess = async (response, file, fileList) => {
  // 处理上传成功后的回调
  // 将上传成功的图片URL保存到表单中
  // dialogForm.imagesUrl = fileList.map(file => file.url);
  const realUrl = response.data;
  if (realUrl) {
    // 更新 dialogForm.imagesUrl
    dialogForm.imagesUrl = [...dialogForm.imagesUrl, realUrl];
    console.log("dialogForm.imagesUrl:", dialogForm.imagesUrl)
  } else {
    console.error("上传接口返回的 url 为空:", response);
  }
  // 更新 fileList，确保显示正确的图片预览
  fileList.forEach((fileItem, index) => {
    if (fileItem.uid === file.uid) {
      fileItem.url = realUrl;
    }
  });
  // fileList.value = fileList.map(fileItem => ({
  //   ...fileItem,
  //   url: realUrl + '?t=' + new Date().getTime() // 添加时间戳防止缓存
  // }));
};
...
const handleRemove = (file, fileList) => {
  // console.log('Before remove:', dialogForm.imagesUrl); // 打印删除前的 imagesUrl

  // 获取被删除文件的 URL 并去除时间戳等额外参数
  let removedUrl = new URL(file.url).pathname; // 使用 URL API 获取路径部分
  // console.log('Removed URL:', removedUrl); // 打印被删除的 URL

  // 从 dialogForm.imagesUrl 中移除该 URL
  dialogForm.imagesUrl = dialogForm.imagesUrl.filter(url => {
    try {
      const cleanUrl = new URL(url).pathname; // 清理 URL
      // console.log('Comparing:', cleanUrl, 'with', removedUrl); // 打印比较的 URL
      return cleanUrl !== removedUrl;
    } catch (error) {
      console.error('Error parsing URL:', url, error);
      return true; // 如果解析失败，保留该 URL
    }
  });

  // 更新 fileList，移除对应的文件项
  fileList.value = fileList.filter(item => {
    try {
      return new URL(item.url).pathname !== removedUrl;
    } catch (error) {
      console.error('Error parsing URL:', item.url, error);
      return true; // 如果解析失败，保留该文件项
    }
  });

  // 打印日志以调试
  // console.log('After remove:', dialogForm.imagesUrl); // 打印删除后的 imagesUrl
};
